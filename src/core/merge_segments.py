...
def merge_segments(asr_data: ASRData, model: str, num_threads: int = 5,
                  max_word_count_cjk: int = 20, max_word_count_english: int = 12) -> ASRData:
    """
    合并ASR数据分段
    ...
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        def process_segment(asr_data_part):
            try:
                ...
                logger.debug(f"LLM处理失败，使用规则based方法进行分割: {str(e)}")
